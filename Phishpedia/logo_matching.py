...
def pred_brand(model, domain_map, logo_feat_list, file_name_list, shot_path: str, gt_bbox, similarity_threshold,
    ...
    # Check if top similarity score meets the main threshold (from config: 0.87)
    # Remove the experimental 0.5 threshold logic that conflicts with the main system
    print(f"[DEBUG] Top similarity score: {top3_simlist[0]:.4f}, threshold: {similarity_threshold}")
    
    # Only proceed if the top score has potential (above a reasonable minimum)
    if top3_simlist[0] < 0.3:
        print("[DEBUG] Top similarity too low (<0.3), skipping brand matching")
    ...
    for j in range(3):
        ...
        # If the largest similarity exceeds the configured threshold
